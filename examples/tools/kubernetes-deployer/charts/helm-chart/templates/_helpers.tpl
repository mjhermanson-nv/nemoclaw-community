...
{{/*
Resolve an explicit OpenShift sandbox UID from the release Namespace. This is
rendered into the gateway's trusted Kubernetes compute-driver configuration,
not the caller-controlled per-sandbox driver_config schema.
*/}}
{{- define "openshell.openshiftSandboxUid" -}}
{{- $uidConfig := .Values.server.openshift.sandboxUid -}}
{{- $sandboxUid := $uidConfig.value -}}
{{- if eq $sandboxUid nil -}}
  {{- $namespace := lookup "v1" "Namespace" "" .Release.Namespace -}}
  {{- if not $namespace -}}
    {{- fail (printf "server.openshift.sandboxUid.enabled=true requires the release namespace %q to exist so Helm can resolve its openshift.io/sa.scc.uid-range annotation; pre-create the namespace or set server.openshift.sandboxUid.value only for offline rendering" .Release.Namespace) -}}
  {{- end -}}
  ...
  {{- if or (le $offset 0) (ge $offset $rangeSize) -}}
    {{- fail (printf "server.openshift.sandboxUid.offset must be greater than zero and less than the namespace SCC UID-range size (%d)" $rangeSize) -}}
  {{- end -}}
  {{- $sandboxUid = add $rangeStart $offset -}}
{{- end -}}
{{- if le (int $sandboxUid) 0 -}}
  {{- fail "server.openshift.sandboxUid.value must be a positive integer when set" -}}
{{- end -}}
{{- int $sandboxUid -}}
{{- end -}}
